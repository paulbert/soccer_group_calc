type ResultRow = [string, number, string, number];
...
const fullSort = (standings: Standings, results: ResultRow[]) => {
  return sortStandings([standings], results, euroSortInstructions);
};

const unsortedStandings = calculateStandings(sampleResults);
// fullSort(unsortedStandings, sampleResults);
console.log(fullSort(unsortedStandings, sampleResults));
